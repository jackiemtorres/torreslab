...
treat <- lmtp_tmle(
  ...
  shift = d1,
  folds = 10, 
  learners_trt = c("SL.glm", "SL.mean", "SL.glmnet", "SL.xgboost"),
...
```{r, depression tmle by hand a different way}
...
ps_model <- glm(A ~ age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, 
                family = binomial, data = cham_household_depress_mean)

g_hat <- predict(ps_model, type = "response")  # Estimated propensity score


q_model <- lm(depress_mc1 ~ A + age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, data = cham_household_depress_mean)
Q_hat <- predict(q_model)  # Initial outcome model predictions



H_AW <- (cham_household_depress_mean$A / g_hat) - ((1 - cham_household_depress_mean$A) / (1 - g_hat))



epsilon_model <- glm(depress_mc1 ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = cham_household_depress_mean)
epsilon <- coef(epsilon_model)


Q_star <- Q_hat + epsilon * H_AW


data_A1 <- cham_household_depress_mean
data_A1$A <- 1
Q1_star <- predict(q_model, newdata = data_A1) + epsilon * ((1 / g_hat) - (0 / (1 - g_hat)))

data_A0 <- cham_household_depress_mean
data_A0$A <- 0
Q0_star <- predict(q_model, newdata = data_A0) + epsilon * ((0 / g_hat) - (1 / (1 - g_hat)))

tmle_estimate <- mean(Q1_star - Q0_star)
tmle_estimate



# Influence function components
IC <- H_AW * (cham_household_depress_mean$depress_mc1 - Q_star) + (Q1_star - Q0_star)

# Standard Error (SE)
SE <- sd(IC) / sqrt(nrow(cham_household_depress_mean))

# 95% Confidence Interval
lower_CI <- tmle_estimate - 1.96 * SE
upper_CI <- tmle_estimate + 1.96 * SE

# Print results
cat("TMLE Estimate:", tmle_estimate, "\n")
...
B <- 500  
tmle_bootstrap <- numeric(B)  


# Bootstrap procedure
set.seed(124)
for (b in 1:B) {
  boot_indices <- sample(1:nrow(cham_household_depress_mean), size = nrow(cham_household_depress_mean), replace = TRUE)  
  boot_data <- cham_household_depress_mean[boot_indices, ] 
  
  # Re-estimate g (propensity score)
  ps_model <- glm(A ~ age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, family = binomial, data = boot_data)
  g_hat <- predict(ps_model, type = "response")
  
  # Re-estimate Q (outcome regression)
  q_model <- lm(depress_mc1 ~ A + age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, data = boot_data)
  Q_hat <- predict(q_model)
  
  # Compute clever covariate
  H_AW <- (boot_data$A / g_hat) - ((1 - boot_data$A) / (1 - g_hat))
  
  # Fit fluctuation model
  epsilon_model <- glm(depress_mc1 ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = boot_data)
  epsilon <- coef(epsilon_model)
  ...
  boot_data_A1 <- boot_data
  boot_data_A1$A <- 1
  Q1_star <- predict(q_model, newdata = boot_data_A1) + epsilon * ((1 / g_hat) - (0 / (1 - g_hat)))

  boot_data_A0 <- boot_data
  boot_data_A0$A <- 0
  Q0_star <- predict(q_model, newdata = boot_data_A0) + epsilon * ((0 / g_hat) - (1 / (1 - g_hat)))

  # Compute TMLE estimate for bootstrap sample
  tmle_bootstrap[b] <- mean(Q1_star - Q0_star)
}

# Compute bootstrap confidence intervals
CI_lower <- quantile(tmle_bootstrap, probs = 0.025)
CI_upper <- quantile(tmle_bootstrap, probs = 0.975)

cat("Bootstrap TMLE Estimate:", mean(tmle_bootstrap), "\n")
cat("95% Bootstrap CI: [", CI_lower, ",", CI_upper, "]\n")
```
...
```{r, sleep tmle another way by hand}
### sleep outcome
cham_household_sleep_mean <- cham_household_analytic %>% filter(!is.na(avg_sleep))
cham_household_sleep_mean <- cham_household_sleep_mean %>% filter(!(is.na(ppb2_mc1) | is.na(age_qx_mc1) | is.na(ageusa_18) | 
                                                                is.na(lang_exam_mc1) | is.na(educcat_mom) | is.na(married) | is.na(pov_binary) | 
...
ps_model <- glm(A ~ age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, 
                family = binomial, data = cham_household_sleep_mean)

g_hat <- predict(ps_model, type = "response")  # Estimated propensity score


q_model <- lm(avg_sleep ~ A + age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
              pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, data = cham_household_sleep_mean)
Q_hat <- predict(q_model)  # Initial outcome model predictions



H_AW <- (cham_household_sleep_mean$A / g_hat) - ((1 - cham_household_sleep_mean$A) / (1 - g_hat))



epsilon_model <- glm(avg_sleep ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = cham_household_sleep_mean)
epsilon <- coef(epsilon_model)
...
data_A1 <- cham_household_sleep_mean
data_A1$A <- 1
Q1_star <- predict(q_model, newdata = data_A1) + epsilon * ((1 / g_hat) - (0 / (1 - g_hat)))

data_A0 <- cham_household_sleep_mean
data_A0$A <- 0
Q0_star <- predict(q_model, newdata = data_A0) + epsilon * ((0 / g_hat) - (1 / (1 - g_hat)))
...
# Influence function components
IC <- H_AW * (cham_household_sleep_mean$avg_sleep - Q_star) + (Q1_star - Q0_star)

# Standard Error (SE)
SE <- sd(IC) / sqrt(nrow(cham_household_sleep_mean))
...
set.seed(124)
for (b in 1:B) {
  boot_indices <- sample(1:nrow(cham_household_sleep_mean), size = nrow(cham_household_sleep_mean), replace = TRUE)  
  boot_data <- cham_household_sleep_mean[boot_indices, ] 
  ...
  # Fit fluctuation model
  epsilon_model <- glm(avg_sleep ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = boot_data)
  epsilon <- coef(epsilon_model)
...
```{r, memory tmle another way by hand}
### sleep outcome
cham_household_mem_mean <- cham_household_analytic %>% filter(!is.na(memory_mc1))
cham_household_mem_mean <- cham_household_mem_mean %>% filter(!(is.na(ppb2_mc1) | is.na(age_qx_mc1) | is.na(ageusa_18) | 
                                                                is.na(lang_exam_mc1) | is.na(educcat_mom) | is.na(married) | is.na(pov_binary) | 
...
ps_model <- glm(A ~ age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
                pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, 
                family = binomial, data = cham_household_mem_mean)

g_hat <- predict(ps_model, type = "response")  # Estimated propensity score


q_model <- lm(memory_mc1 ~ A + age_qx_mc1 + ageusa_18 + lang_exam_mc1 + educcat_mom + married +
              pov_binary + work_mc1 + lvhome_n18_mc1 + menoind_mc1 + alc, data = cham_household_mem_mean)
Q_hat <- predict(q_model)  # Initial outcome model predictions



H_AW <- (cham_household_mem_mean$A / g_hat) - ((1 - cham_household_mem_mean$A) / (1 - g_hat))



epsilon_model <- glm(memory_mc1 ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = cham_household_mem_mean)
epsilon <- coef(epsilon_model)
...
data_A1 <- cham_household_mem_mean
data_A1$A <- 1
Q1_star <- predict(q_model, newdata = data_A1) + epsilon * ((1 / g_hat) - (0 / (1 - g_hat)))

data_A0 <- cham_household_mem_mean
data_A0$A <- 0
Q0_star <- predict(q_model, newdata = data_A0) + epsilon * ((0 / g_hat) - (1 / (1 - g_hat)))
...
# Influence function components
IC <- H_AW * (cham_household_mem_mean$memory_mc1 - Q_star) + (Q1_star - Q0_star)

# Standard Error (SE)
SE <- sd(IC) / sqrt(nrow(cham_household_mem_mean))
...
set.seed(124)
for (b in 1:B) {
  boot_indices <- sample(1:nrow(cham_household_mem_mean), size = nrow(cham_household_mem_mean), replace = TRUE)  
  boot_data <- cham_household_mem_mean[boot_indices, ] 
  ...
  # Fit fluctuation model
  epsilon_model <- glm(memory_mc1 ~ -1 + offset(Q_hat) + H_AW, family = gaussian, data = boot_data)
  epsilon <- coef(epsilon_model)
...
cham_household_med
# calculate number of individuals missing neurocog outcomes                                                       
cham_household_med %>% filter(!is.na(memory_mc1))   
cham_household_med %>% filter(!is.na(global_rc_mc1))  
```
...
example_data <- make_example_data(1000)
...
de <- medoutcon(
  ...
  Z = cham_household_med_mem[, ..z_names],
