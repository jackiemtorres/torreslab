...
mi.mean.se.sd.by <- function(IMP, X, BY) {
  ...
  for(i in 1:length(LEVELS)) {
    ...
    POOLED    <- pool.scalar(Q = MEAN,
                             ...
                             n = N[i])
    if (i == 1) {
      MEAN.SE.SD <- c(            POOLED$qbar, sqrt(POOLED$t), mean(SD), POOLED$ubar)
    } else {
      MEAN.SE.SD <- c(MEAN.SE.SD, POOLED$qbar, sqrt(POOLED$t), mean(SD), POOLED$ubar)
    }
  ...
  names(OUT) <- c(rbind(paste("mean", 1:length(LEVELS), sep="."),
                        ...
                        paste("mean.var",   1:length(LEVELS), sep=".")))
  rownames(OUT) <- X
  return(OUT)
}
